...
For the canonical graphical notation for event occurrences and participants,
see **Figure 13** (Event Occurrences) and **Figure 22** (Flows) in SysML v2.0,
which cover the `message` form and its visual representation.
...
Self-messages in the standard UML sequence diagram notation leave and re-enter
the same lifeline as a U-shaped arc on the right side.

---

## 9. Loop combined fragments
...
## 10. opt / par / ref combined fragments

UML combined fragments beyond `alt` can be approximated in SysML v2:

- **`opt`** — an `if` block without an `else` branch (§7.17.11).
- **`par`** — parallel message exchanges modelled with `fork`/`join` action
  structure (§7.17.3).
- **`ref`** — referencing another interaction definition via `perform action`
  typed by another `action def` (§7.17.6).
...
## 11. Create and destroy lifelines
...
destruction points).  In SysML v2 these are modelled via `SendActionUsage` and
`AcceptActionUsage` targeting creation or termination events.
...
|---|---|---|
| Occurrences (`occurrence def`) | §7.9 | Figure 12 |
| Event Occurrence Usages (`event occurrence`) | §7.9.5 | Figure 13 |
| Parts as participants (`part def`, `part`) | §7.11 | Figure 15 |
| Flows and Messages — `message` keyword and syntax | §7.16 | Figure 22 |
...
| Accept Action Usages (`accept when <trigger>`) | §7.17.8 | Figure 26 |
| Terminate Action Usages | §7.17.10 | Figure 28 |
| If Action Usages (`if … else …` → `alt` fragment) | §7.17.11 | Figure 29 |
